    ...
    def __init__(self):
        try:
            self.lstm_model = load_model(
                _model_file("final_forthesis_bidirectionallstm_and_encoders_exercise_classifier_model.h5")
            )
        ...
        try:
            self.scaler = joblib.load(_model_file("thesis_bidirectionallstm_scaler.pkl"))
        except Exception as e:
            print(f"Error loading scaler: {e}")
            self.scaler = None
        
        try:
            self.label_encoder = joblib.load(_model_file("thesis_bidirectionallstm_label_encoder.pkl"))
        ...
        if is_video:
            ...
            while cap.isOpened():
                ...
                if current_time - last_update_time >= update_interval:
                    stframe.image(img, channels='BGR', width="stretch")
